...
namespace libDB
{
    public class MySqlserverDB : IDB
    {
        ...
        /// 
        public MySqlserverDB(string server, string database, string user, string password)
        {
            string cs = "Server =" + server + "; Database =" + database + "; User Id =" + user + "; Password =" + password + ";";
            cn = new MySqlConnection(cs);

        }//cierra construsctor de MyMySqlServerDB

        public MySqlserverDB()
        {
            cn = new MySqlConnection("Server=localhost;Database=pruebaorm; User Id=root; password=ccmc1503");
        }

        /// <summary>
        /// Abre la conexión establecida previamente
        /// </summary>
        /// <returns>True: si se abre, false: si hay error. (Consultar variable MyMySqlserver errorMsg.Message)</returns>
        public bool abrir()
        {
            bool res = false;
            try
            ...
            {
                errorMsg = "Error MyMySql al abrir la conexión.\n" + MySqle.Message + "\n" + MySqle.Number;
            }
            catch (Exception ex)
            {
        ...
        public bool cerrar()
        {
            ...
            {
                errorMsg = "Error MySql al cerrar conexión.\n" + MySqle.ToString();
            }
        ...
        bool IDB.insertar(string tabla, string campos, string valores)
        {
            ...
            {
                errorMsg = "Error MySql al insertar.\n " + MySqle.Number;
                numError = MySqle.Number;
            }
            catch (Exception ex)
            {
        ...
        bool IDB.modificar(string tabla, string campoValores,string campoWhere, string where)
        {
            ...
            {
                abrir();
                cmd = new MySqlCommand("UPDATE " + tabla + " SET " + campoValores + "  WHERE "+campoWhere+"= " + where + ";", cn);
                cmd.ExecuteNonQuery();
                res = true;
            ...
            {
                errorMsg = "Error MySql al modificar.\n " + MySqle.ToString();
            }
            catch (Exception ex)
            {
        ...
        bool IDB.borrar(string tabla, string campoWhere, string id)
        {
            ...
            {
                abrir();
                cmd = new MySqlCommand("DELETE FROM " + tabla + " WHERE "+campoWhere+"= " + id + ";", cn);
                cmd.ExecuteNonQuery();
                res = true;
            ...
            {
                errorMsg = "Error MySql al borrar registro.\n" + MySqle.ToString();
            }
            catch (Exception ex)
            {
        ...
        bool IDB.borradoLogico(string tabla, string campo, string valor, string id)
        {
            ...
            {
                abrir();
                cmd = new MySqlCommand("UPDATE " + tabla + " SET " + campo + "=" + valor + " WHERE id=" + id + ";", cn);
                cmd.ExecuteNonQuery();
                res = true;
            }
            catch (MySqlException MySqle)
            {
                errorMsg = "Error MySql al realizar el borrado.\n " + MySqle.Message;
            }
            catch (Exception ex)
            {
        ...
        DataTable IDB.consultaTodo(string campos,string tabla)
        {
            ...
            try
            {
                ...
                if (abrir() == false)
                {
                    errorMsg = "La conoxión de Mysql no pudo abrirse";
                }
                else
                {
                    da = new MySqlDataAdapter("SELECT "+campos+" FROM " + tabla + ";", cn);
               ...
               // cmd = new MySqlCommand("SELECT * FROM " + tabla + ";", cn);
                //cmd.ExecuteReader();
            ...
            {
                errorMsg = "Error MySql al cargar los datos. \n" + MySqle.Message;
            }
            catch (Exception ex)
            {
        ...
        object IDB.consultaUnValor(string tabla, string campo, string id)
        {
            ...
            {
                abrir();
                cmd = new MySqlCommand("SELECT " + campo + " FROM " + tabla + " WHERE id=" + id + ";");
        ...
        DataTable IDB.consultaUnRegistro(string campo, string tabla, string campoWhere, string id)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    da = new MySqlDataAdapter ("SELECT " + campo + " FROM " + tabla + " WHERE " + campoWhere + "= " + id + " ;", cn);
                    da.Fill(dt);
                    if (dt.Rows.Count <=0)
                    {
                        dt = null;
        ...
        DataTable IDB.consultaVariosDt(string tabla, string campos, string campoCondicion, string where)
        {
            ...
            {
                abrir();
                da = new MySqlDataAdapter("SELECT " + campos + " FROM " + tabla + " WHERE " + campoCondicion + "= " + where + ";", cn);
                //cmd.ExecuteReader();
            ...
            {
                errorMsg = "Error en la MySql server. \n" + MySqle.Message;
            }
            catch (Exception ex)
            {
        ...
        DataTable IDB.consultaVarios(string tabla, string campos, string where)
        {
            ...
            {
                abrir();
                cmd = new MySqlCommand("SELECT " + campos + " FROM " + tabla + " WHERE id=" + where + ";", cn);
                cmd.ExecuteReader();
        ...
        bool IDB.autenticar(string campos, string tabla, string campo1, string valor1, string campo2, string valor2)
        {
            ...
            {
                abrir();
                cmd = new MySqlCommand("SELECT "+campos+" FROM "+tabla+" WHERE "+campo1+"='"+valor1+"' AND "+campo2+"= '"+valor2+"';", cn);
                ...
                if (dr.HasRows)
                {
                    ...
                    if (dr[7].ToString() != "")
                    {
                        idMaestro = Convert.ToInt32(dr[7].ToString());
        ...
        DataTable IDB.buscarDato(string campo, string tabla, string campoWhere, string valor)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    da = new MySqlDataAdapter("SELECT " + campo + " FROM " + tabla + " WHERE " + campoWhere + " LIKE '" + valor + "%';", cn);
                    da.Fill(dt);
                    if (dt.Rows.Count <= 0)
                    {
                ...
                //cmd = new MySqlCommand("SELECT " + campo + " FROM " + tabla + " WHERE " + campoWhere + " LIKE '" + valor + "%';", cn);
                //cmd.ExecuteReader();
        ...
        bool IDB.consultaUnDato(string campo, string tabla, string campo1, string valor1, string campo2, string valor2)
        {
            ...
            try
            {
                ...
                {
                    cmd = new MySqlCommand("SELECT " + campo + " FROM " + tabla + " WHERE " + campo1 + "= " + valor1 + " AND "+campo2+"= '"+valor2+"' ;)", cn);
                    dr = cmd.ExecuteReader();
                    dr.Read();
        ...
        bool IDB.consultaUnDato2(string campo, string tabla, string where, string valorWhere)
        {
            ...
            try
            {
                ...
                {
                    cmd = new MySqlCommand("SELECT " + campo + " FROM " + tabla + " WHERE " + where + "=" + valorWhere + ";",cn);
                    dr = cmd.ExecuteReader();
                    dr.Read();
                    if (dr.HasRows)
                    {
        ...
        List<object[]> IDB.consultaParaCombo(string campos, string tabla)
        {
            ...
            try
            {
                ...
                {
                    cmd = new MySqlCommand("SELECT " + campos + " FROM " + tabla + ";", cn);
